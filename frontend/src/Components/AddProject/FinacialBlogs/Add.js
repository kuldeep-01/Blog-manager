...
function Add() {
  ...
  const [pic, setPic] = useState(
    "https://images.pexels.com/photos/210661/pexels-photo-210661.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
  );
  ...
  useEffect(() => {
    if (url) {
      fetch("http://localhost:5000/createFinancial", {
        method: "post",
        headers: {
  ...
  const post = () => {
    ...
    data.append("upload_preset", "t9563ouf")
    data.append("cloud_name", "dqiuxctub")
    fetch("https://api.cloudinary.com/v1_1/dqiuxctub/image/upload", {
      method: "post",
      body: data,
    })
      .then((res) => res.json())
